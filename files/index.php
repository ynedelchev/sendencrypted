...
function del() {
  ...
  if ($dh = opendir($dir)) {
    while (($file = readdir($dh)) !== FALSE) {
      ...
      if ($content === FALSE) {
        error_log("Content of file \"".$dir.'/'.$file."\" cannot be read. Thus not deleting it as part of delete expired files operation.");
      ...
      if ($meta === NULL) {
        ...
        error_log("Content of file \"".$dir.'/'.$file."\" cannot be JSON decoded or the encoded data is deeper than the recursion limit ($recursion_limit). Thus not deleting it as part of delete expired files operation.");
        $failed[$id] = "Cannot parse metadata.";
        continue;
      } 
      if (!isset($meta['status'])) {
        error_log("Metadata file \"".$dir.'/'.$file."\" does not specify \"status\" property, thus it is unknown what the status of the corresponding data file is. Not deleting it as part of delete expired files operation. Content of metadata file is: ".json_encode($meta));
        $failed[$id] = "Cannot find status.";
        continue;
      }
      if (!isset($meta['expires'])) {
        error_log("Metadata file \"".$dir.'/'.$file."\" does not specify \"expires\" property, thus it is unknown when the corresponding data file expires. Not deleting it as part of delete expired files operation. Content of metadata file is: ".json_encode($meta));
      ...
      if ($meta['status'] == "unknown") {
        error_log("Status defined in metadata file \"".$dir.'/'.$file."\" is \"".$meta["status"]."\", so it would not be deleted.");
      ...
      if ($now > $expires) {
        ...
        $msgmeta = "Deleting \"".$dir.'/'.$file."\" expired on ".$expired.". Now : ".$now.".";
        $msgdata = "Deleting \"".$dir.'/'.$data."\" expired on ".$expired.". Now : ".$now.".";
        $successdata = FALSE;
        $successmeta = FALSE;
        error_log($msgdata);
        if (file_exists($dir.'/'.$data)) {
          $successdata = unlink($dir.'/'.$data);
          if ($successdata === FALSE) {
            error_log("Failed   \"".$dir.'/'.$data."\" deletion.");
        ...
        if (file_exists($dir.'/'.$file)) {
          $successmeta = unlink($dir.'/'.$file);
          if ($successmeta === FALSE) {
            error_log("Failed   \"".$dir.'/'.$file."\" deletion.");
    ...
    http_exit(500, "Вътрешносървърна грешка", "Internal Server Error. Cannot open directory \"".$dir."\" for reading. Cannot process data and metadata files in this directory.", null);
  }
}   
